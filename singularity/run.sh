...
#SBATCH --array=0-4                             # Only use this if you want the cluster to automaticall start multiple jobs, which all run the script below, but with a different argument. See also: https://support.ceci-hpc.be/doc/_contents/QuickStart/SubmittingJobs/SlurmTutorial.html#embarrassingly-parallel-workload-example-job-array

module reset
module load system singularity

singularity exec singularity_container.sif --nv bash -c "./run_in_container.sh $SLURM_ARRAY_TASK_ID"
